...
<?php
  ...
  function sqlquery($tipo, $idref, $nivelacesso) {    
    $sql="";
    if ( $tipo == 'setor' ) {
      $sql = "SELECT id_setor, depto, sigla, endereco, bairro, telefone, email, id_referencia, observacao FROM tb_setor ORDER BY depto";
      
    }
    elseif ( $tipo == 'funcionario' ) {
      $sql = "SELECT nome, matricula, cargo, tipocontratacao, email, telefone, whatsapp, slack, trello, observacao, id_referencia FROM tb_funcionario ORDER BY nome";
    }
    elseif ( $tipo == 'solicitacao' ) {
      $sql = "SELECT so.`data` as `Data_Solicitacao`, (SELECT `status` from  tb_status WHERE id_status = so.`status`) as `Status`, so.`status_setor` as `Parado_Setor`, 
